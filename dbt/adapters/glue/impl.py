...
# Apply custom location if specified
custom_location = config.get('custom_location', '')
if custom_location:
    writer = writer.option("path", custom_location)

# Save the table
writer.saveAsTable(schema + "." + model_name)

# Refresh the table to make it available
spark.sql("REFRESH TABLE " + schema + "." + model_name)

print("DEBUG: Successfully wrote table " + schema + "." + model_name)
